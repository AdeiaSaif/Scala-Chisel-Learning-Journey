...
class ex1test extends FreeSpec with ChiselScalatestTester{
    "ALU test "in{
        test(new ex1) {c=>
            val array_op=Array(ALU_ADD,ALU_SUB,ALU_AND,ALU_OR,ALU_XOR,ALU_SLT,ALU_SLL,ALU_SLTU,ALU_SRL,ALU_SRA,ALU_COPY_A,ALU_COPY_B, ALU_beq, ALU_bne, ALU_blt, ALU_bge)   

        for(i<-0 until 100){
            val src_a=Random.nextLong()& 0xFFFFFFFFL
            val src_b=Random.nextLong()& 0xFFFFFFFFL
            val opr=Random.nextInt(16)
